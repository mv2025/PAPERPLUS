...
const HorizontalProcess = () => {
  ...
  return (
    <section className="py-10 sm:py-14 bg-[#FAF6EC] border-t border-[#E8E1D0] text-[#141F42]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        
        {/* Section Header (Compact & Proportional) */}
        <div className="text-center max-w-2xl mx-auto mb-6 sm:mb-8">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#DA2030]/10 text-[#DA2030] text-[11px] font-extrabold uppercase tracking-widest mb-2 border border-[#DA2030]/20">
            <Sparkles className="w-3 h-3" /> Craftsmanship Masterclass
          </span>
          <Typography variant="h2" className="text-[#141F42] text-2xl sm:text-3xl lg:text-3xl font-extrabold mb-2 leading-tight">
            How Every <span className="text-[#DA2030]">Paper Plus</span> Calendar is Engineered
          </Typography>
          <p className="text-[#5C6478] text-xs sm:text-sm leading-relaxed">
            From raw 250 GSM stock and hot-stamped gold foil to precision 6-color presses and steel Wire-O assembly.
          </p>
        </div>

        {/* Interactive Step Switcher Tabs (Compact Grid) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 mb-6">
          {PROCESS_STEPS.map((step, idx) => (
            <button
              key={step.id}
              onClick={() => setActiveStep(idx)}
              className={`p-2.5 sm:p-3 rounded-xl border text-left transition-all duration-300 flex flex-col justify-between ${
                activeStep === idx
                  ? 'bg-[#141F42] text-white border-[#141F42] shadow-md scale-[1.01]'
                  : 'bg-white text-[#141F42] border-[#E8E1D0] hover:border-[#DA2030]/50 hover:bg-white/90'
              }`}
            >
              <div className="flex items-center justify-between w-full mb-1.5">
                <span className={`text-[10px] font-extrabold uppercase tracking-wider ${activeStep === idx ? 'text-[#DA2030]' : 'text-[#8D96A8]'}`}>
                  {step.stage}
                </span>
                <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-black ${activeStep === idx ? 'bg-[#DA2030] text-white' : 'bg-[#FAF6EC] text-[#141F42] border border-[#E8E1D0]'}`}>
                  0{idx + 1}
                </span>
              </div>
        ...
        {/* Active Stage Interactive Showcase Card (Compact & Balanced) */}
        <div className="bg-white border border-[#E8E1D0] rounded-2xl p-5 sm:p-8 shadow-lg">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-7 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="px-2.5 py-0.5 rounded-full bg-[#DA2030] text-white text-[10px] font-extrabold uppercase tracking-wider shadow-sm">
                    {current.stage}
                  </span>
                ...
                <h3 className="text-lg sm:text-xl md:text-2xl font-extrabold text-[#141F42] mb-2 leading-snug">
                  {current.title}
                </h3>

                <p className="text-[#5C6478] text-xs sm:text-sm leading-relaxed mb-4">
                  {current.desc}
                </p>
                ...
                <div className="space-y-1.5 mb-4">
                  {current.highlights.map((h, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <div className="w-4 h-4 rounded-full bg-[#FAF6EC] border border-[#E8E1D0] flex items-center justify-center flex-shrink-0 mt-0.5 text-[#DA2030]">
                        <CheckCircle2 className="w-3 h-3" />
                      </div>
                      <span className="text-xs font-medium text-[#141F42] leading-tight">
                        {h}
                      </span>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-4 border-t border-[#E8E1D0]">
                {current.specs.map((spec, i) => (
                  <div key={i} className="bg-[#FAF6EC] border border-[#E8E1D0] p-2 rounded-lg">
                    <span className="text-[9px] uppercase font-bold text-[#8D96A8] block">
                      {spec.label}
                    </span>
                    <span className="text-[11px] font-extrabold text-[#141F42] block truncate mt-0.5">
                      {spec.value}
                    </span>
                  </div>
                ))}
            ...
            <div className="lg:col-span-5 flex flex-col items-center justify-center">
              <div className="w-full bg-[#FAF6EC] border border-[#E8E1D0] rounded-xl p-4 relative overflow-hidden shadow-inner flex items-center justify-center h-[220px] sm:h-[260px] group">
                <span className="absolute top-2.5 left-2.5 bg-[#141F42] text-white text-[9px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider shadow">
                  {current.badge}
                </span>
                <img 
                  src={current.img} 
                  alt={current.title} 
                  className="w-full h-full object-contain drop-shadow-xl group-hover:scale-105 transition-transform duration-500 rounded-lg"
                />
              </div>
              ...
              <div className="flex items-center justify-between w-full mt-4 px-1">
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-white border border-[#E8E1D0] text-xs font-bold text-[#141F42] hover:border-[#DA2030] hover:text-[#DA2030] transition-colors flex items-center gap-1 shadow-sm"
                >
                  ← Previous
                </button>
                <div className="flex gap-1.5">
                  {PROCESS_STEPS.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all ${activeStep === i ? 'w-5 bg-[#DA2030]' : 'w-1.5 bg-[#E8E1D0]'}`}
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-[#141F42] text-white text-xs font-bold hover:bg-[#DA2030] transition-colors flex items-center gap-1 shadow-sm"
                >
                  Next →
                </button>
              </div>
